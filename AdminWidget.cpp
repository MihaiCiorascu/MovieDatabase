...
QHBoxLayout *AdminWidget::createUndoRedoButtons() {
    auto *undoRedoButtons = new QHBoxLayout();
    this->undoButton = new QPushButton("Undo", this);
    this->redoButton = new QPushButton("Redo", this);

    undoRedoButtons->addWidget(undoButton);
    undoRedoButtons->addWidget(redoButton);
...
AdminWidget::AdminWidget(Service &service, QWidget *parent) : service(service), QWidget(parent) {
    ...
    QHBoxLayout *undoRedoButtons = createUndoRedoButtons();

    MovieWatchListModel* model = new MovieWatchListModel(service, this);
    QTableView* view = new QTableView(this);
    view->setModel(model);
    view->resize(200, 200); // Set the size as needed
    view->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding); // Allow the table to expand
    ...
    movieInformation->addLayout(undoRedoButtons);
    this->createTableWidget();
    adminLayout->addWidget(movieTable);
    adminLayout->addWidget(view);
