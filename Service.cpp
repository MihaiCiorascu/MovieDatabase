...
void Service::addMovie(Movie movie) {
    ...
    auto command = std::make_unique<AddCommand>(*this->repository, movie);
...
void Service::removeMovieByPosition(int positionOfMovie) {
    ...
    Movie movie = this->repository->getMovieByPosition(positionOfMovie);
    this->repository->removeMovieByPosition(positionOfMovie);

    auto command = std::make_unique<RemoveCommand>(*this->repository, movie);

    undoStack.push_back(std::move(command));
    redoStack.clear();
}

void Service::updateMovie(int position, Movie new_movie) {
    if (!this->repository->validatePosition(position))
        throw InvalidPositionException();
    if (this->repository->returnPosition(new_movie) != Positions::INDEX_NOT_FOUND)
        throw DuplicateMovieException();
    Movie old_movie = this->repository->getMovieByPosition(position);
    this->repository->updateMovie(position, new_movie);

    auto command = std::make_unique<UpdateCommand>(*this->repository, old_movie, new_movie, position);
